...
const useServerStore = create<ServerState>((set) => ({
    ...
    addRoomParticipant: (name) => set(state => {
        const room = state.rooms.find(c => c.name === name);
        if (!room) return { rooms: [...state.rooms, { name, participantsCount: 1 }] };
        return {
            rooms: state.rooms.map(c => c.name === name ? { ...c, participantsCount: c.participantsCount + 1 } : c)
        }
    }),
    removeRoomParticipant: (name) => set(state => {
        const room = state.rooms.find(c => c.name === name);
