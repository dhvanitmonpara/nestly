  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger className="rounded-md w-full p-1 flex space-x-2 cursor-pointer bg-zinc-800 hover:bg-zinc-700/40 transition-colors border-zinc-700 border absolute bottom-1 left-0">
        <div style={{ color: `#${user?.accentColor}` }} className="bg-zinc-900 rounded-full w-10 h-10 flex items-center justify-center">
          {user?.displayName.split("")[0].toUpperCase() ?? "U"}
        </div>
        <div>
          <p className="text-start text-sm text-zinc-300 font-semibold">{user?.displayName}</p>
          <p className="text-xs text-start text-zinc-400">{user?.email}</p>
        </div>
      </DropdownMenuTrigger>
