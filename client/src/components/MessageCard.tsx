...
function MessageCard({ chat, continuesMessage = false }: { chat: IMessage, continuesMessage?: boolean }) {
    const color = `#${chat.user?.accent_color}`
    return (
        <div className={`flex space-x-2 ${continuesMessage ? "py-0.5" : "pt-2 mt-2 border-t border-zinc-800"} mx-6 group`} key={chat.id}>
            {/* <img src={chat.user?.username.slice(0, 2)} alt={chat.user?.username} className="w-10 h-10 rounded-full" /> */}
            {continuesMessage
                ? <div className="w-8 text-[0.60rem] opacity-0 group-hover:opacity-100 text-gray-400 line-clamp-1">{chat.createdAt ? formatTimestamp(chat.createdAt) : ""}</div>
            ...
            <div className="w-full">
                ...
                <div className="text-zinc-200 text-sm flex justify-between">
                    ...
                    {continuesMessage && <div className="space-x-3 flex items-center">
                        <div className="divide-x divide-zinc-700">
                            <button className="text-zinc-500 text-xs hover:text-zinc-300 p-1 rounded-l-md opacity-0 group-hover:opacity-100 transition-all bg-zinc-800 hover:bg-zinc-700 cursor-pointer">
                                <MdEdit />
                            </button>
